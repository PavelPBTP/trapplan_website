...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const canonical = new URL(withLocale(locale, "/game-launch-blueprint"), origin).toString();
  const languages = Object.fromEntries(
    SUPPORTED_LOCALES.map((l) => [l, new URL(withLocale(l, "/game-launch-blueprint"), origin).toString()]),
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...languages,
        "x-default": new URL(withLocale("en", "/game-launch-blueprint"), origin).toString(),
      },
    },
    openGraph: {
      type: "website",
...
function SectionKicker({ children }: { children: string }) {
  return (
    <div className="text-[12px] font-semibold tracking-wide text-tertiary">{children}</div>
  );
}
        ...
        <div className="mx-auto max-w-[1200px] px-6 py-16 lg:px-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12">
            ...
            <div className="lg:col-span-8">
              <div className="mb-8 aspect-[16/9] w-full overflow-hidden rounded-[24px] border border-[rgba(244,241,234,0.1)] bg-card">
                <div className="h-full w-full p-6">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-3">
                    <div className="h-full rounded-[18px] border border-[rgba(244,241,234,0.1)] bg-card p-4">
                      <div className="h-2 w-16 rounded-full bg-[rgba(244,241,234,0.1)]" />
                      <div className="mt-3 h-2 w-full rounded-full bg-[rgba(244,241,234,0.1)]" />
                      <div className="mt-2 h-2 w-2/3 rounded-full bg-[rgba(244,241,234,0.1)]" />
                    </div>
                    <div className="h-full rounded-[18px] border border-[rgba(244,241,234,0.1)] bg-card p-4">
                      <div className="h-2 w-14 rounded-full bg-[rgba(244,241,234,0.1)]" />
                      <div className="mt-3 h-2 w-full rounded-full bg-[rgba(244,241,234,0.1)]" />
                      <div className="mt-2 h-2 w-1/2 rounded-full bg-[rgba(244,241,234,0.1)]" />
                    </div>
                    <div className="h-full rounded-[18px] border border-[rgba(244,241,234,0.1)] bg-card p-4">
                      <div className="h-2 w-18 rounded-full bg-[rgba(244,241,234,0.1)]" />
                      <div className="mt-3 h-2 w-full rounded-full bg-[rgba(244,241,234,0.1)]" />
                      <div className="mt-2 h-2 w-3/5 rounded-full bg-[rgba(244,241,234,0.1)]" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-8">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {buildTogetherItems.map((item) => (
                  <div
                    key={item.label}
                    className="rounded-[18px] border border-[rgba(244,241,234,0.1)] bg-card p-4 shadow-[0_12px_30px_rgba(0,0,0,0.05)]"
                  >
                    <div className="aspect-[16/9] w-full overflow-hidden rounded-[14px] border border-[rgba(244,241,234,0.1)] bg-card">
                      <div className="h-full w-full p-3">
                        <div className="h-2 w-20 rounded-full bg-[rgba(244,241,234,0.1)]" />
                        <div className="mt-3 h-2 w-full rounded-full bg-[rgba(244,241,234,0.1)]" />
                        <div className="mt-2 h-2 w-3/4 rounded-full bg-[rgba(244,241,234,0.1)]" />
                      </div>
                    </div>

                    <div className="mt-4 flex items-start gap-3">
                      <span className="mt-[6px] h-2 w-2 flex-none rounded-full bg-[var(--accent)]/70" />
                      <div className="text-[14px] font-semibold leading-6 text-secondary">{item.label}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="relative overflow-hidden bg-card">
        ...
        <div className="mx-auto max-w-[1200px] px-6 py-16 lg:px-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12">
            ...
            <div className="lg:col-span-8">
              <div className="relative rounded-[24px] border border-[rgba(244,241,234,0.1)] bg-card px-7 py-7 shadow-[0_18px_50px_rgba(0,0,0,0.06)]">
                <div className="absolute left-[34px] top-7 bottom-7 w-px bg-[rgba(244,241,234,0.1)]" aria-hidden />

                <div className="space-y-7">
                  <div className="grid grid-cols-[28px,1fr] gap-5">
                    <div className="pt-[2px]">
                      <span className="mt-[3px] block h-3 w-3 rounded-full bg-[var(--accent)]" aria-hidden />
                    </div>
                    <div>
                      <div className="text-[12px] font-semibold tracking-wide text-tertiary">
                        {t(locale, "game_launch_blueprint.ui.best_fit.status_label")}
                      </div>
                      <div className="mt-2 text-[15px] leading-7 text-secondary">
      ...
      <section className="bg-void-alt">
        <div className="mx-auto max-w-[1200px] px-6 py-16 lg:px-10">
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="rounded-[24px] border border-[rgba(244,241,234,0.1)] bg-card px-7 py-7 shadow-[0_20px_50px_rgba(0,0,0,0.06)]">
              <div className="text-[12px] font-semibold tracking-wide text-tertiary">
                {t(locale, "game_launch_blueprint.ui.process.step1.label")}
              </div>
              <div className="mt-3 text-[16px] font-extrabold tracking-tight text-bone">
