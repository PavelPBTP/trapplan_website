(function () {
  ...
  function i18n(key, fallback) {
    try {
      const dict =
        window && window.__TP_I18N && window.__TP_I18N.steam_pricing_planner
          ? window.__TP_I18N.steam_pricing_planner
          : null;
  ...
  function setStatus(msg, type) {
    ...
    if (box) {
      if (type === "error") {
        ...
        box.textContent =
          FX_ERROR ||
          i18n("status_could_not_load_fx", "Could not load FX rates.");
      } else {
              ...
              <table style="width:100%; border-collapse: collapse; font-size: 13px;">
                <thead>
                  <tr style="background: rgba(244,241,234,0.06); text-align:left;">
                    ...
                    <th style="padding: 12px;">${escapeHtml(i18n("table_sale_price", "Sale Price"))}</th>
                  </tr>
                </thead>
  ...
  function computeBaseUsd() {
    ...
    const reason = override
      ? i18n("reason_manual_setting", "Manual setting.")
      : i18n("reason_based_on_genre_length", "Based on genre and length.");
  ...
  function renderRows({ usdBase, filterText }) {
    ...
    tbody.innerHTML =
      rows.join("") ||
      `<tr><td colspan="4" style="padding:20px; text-align:center;">${escapeHtml(i18n("empty_no_countries", "No countries found."))}</td></tr>`;
  }
  ...
  async function calculate() {
    ...
    try {
      setStatus(i18n("status_syncing_fx", "Syncing FX..."), "info");
      const fx = await fetchFxRatesMulti("USD");
      FX = fx.rates;
      FX_SOURCE = fx.source;
      setStatus(
        i18n("status_fx_source", "Source: {source}").replace(
          "{source}",
          String(FX_SOURCE || ""),
        ),
        "info",
      ...
      setStatus(i18n("status_fx_offline", "FX Offline"), "error");
      renderRows({
        usdBase: usd,
        filterText: document.getElementById("tp-filter")?.value,
      });
